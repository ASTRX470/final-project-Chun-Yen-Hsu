...
//!  The method calculates the corner eh
...
void FieldDiffusion::HallEMF(const FaceField &b, const AthenaArray<Real> &bc,
  ...
  if (pmb->block_size.nx2 == 1) {
    for (int i=is; i<=ie+1; ++i) {
        ...
        Real Bsq   = SQR(intBx) + SQR(intBy) + SQR(intBz) + TINY_NUMBER;
        Real JcurlB_1 = J2(ks,js,i)*intBz/sqrt(Bsq) - J3(ks,js,i)*intBy/sqrt(Bsq);
        Real JcurlB_2 = J3(ks,js,i)*intBx/sqrt(Bsq);
        //Real JcurlB_2 = J3(ks,js,i)*intBx/sqrt(Bsq) - J1(ks,js,i)*intBz/sqrt(Bsq);
        Real JcurlB_3 = - J2(ks,js,i)*intBx/sqrt(Bsq);
        //Real JcurlB_3 = J1(ks,js,i)*intBy/sqrt(Bsq) - J2(ks,js,i)*intBx/sqrt(Bsq);
        
        e2(ks  ,js  ,i) += eta_H * JcurlB_2;
        e2(ke+1,js  ,i)  = e2(ks,js,i);
        e3(ks  ,js  ,i) += eta_H * JcurlB_3;
        e3(ks  ,je+1,i)  = e3(ks,js,i);
    }
    return;
  }
  ...
  #pragma omp simd
        for (int i=is; i<=ie+1; ++i) {
          // emf.x
            ...
            Real Bsq   = SQR(intBx) + SQR(intBy) + SQR(intBz) + TINY_NUMBER;
            Real JcurlB_1 = intJy*intBz/sqrt(Bsq) - intJz*intBy/sqrt(Bsq);

            e1(ks  ,j,i) += eta_H * JcurlB_1;
            e1(ke+1,j,i)  = e1(ks,j,i);
          ...
          Bsq   = SQR(intBx) + SQR(intBy) + SQR(intBz) + TINY_NUMBER;
          Real JcurlB_2 = intJz*intBx/sqrt(Bsq) - intJx*intBz/sqrt(Bsq);

          e2(ks  ,j,i) += eta_H * JcurlB_2;
          e2(ke+1,j,i)  = e2(ks,j,i);
          ...
          Bsq   = SQR(intBx) + SQR(intBy) + SQR(intBz) + TINY_NUMBER;
          Real JcurlB_3 = intJx*intBy/sqrt(Bsq) - intJy*intBx/sqrt(Bsq);

          e3(ks,  j,i) += eta_H * JcurlB_3;
          e3(ke+1,j,i) = e3(ks,j,i);
        }
      }
      return;
  ...
  #pragma omp simd
        for (int i=is; i<=ie+1; ++i) {
          ...
          e3(k,j,i) += eta_H * JcurlB_3;
          //std::cout<<"eta_hall = "<<eta_H<<std::endl;
          //std::cout<<"JcurlB_3 = "<<JcurlB_3<<std::endl;
          //std::cout<<"eta_hall_e3 = "<<e3(k,j,i)<<std::endl;
        }
      }
    }
  return;
