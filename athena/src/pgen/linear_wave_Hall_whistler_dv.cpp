...
Real d0, p0, u0, bx0, by0, bz0, dby, dbz, dv0;
Real k_wave, sigma, omega, CA0;
int wave_flag;
...
// 20240414 Chun-Yen
Real eta_hall;
void Hall_coefficient(
    FieldDiffusion *pfdif, MeshBlock *pmb, const AthenaArray<Real> &w,
    const AthenaArray<Real> &bmag, int is, int ie, int js, int je, int ks, int ke) {
  auto Hall = FieldDiffusion::DiffProcess::hall;
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
          pfdif->etaB(Hall, k, j, i) = eta_hall;
            //std::min(eta_cap, q_ad * SQR(bmag(k, j, i)) / sqrt(d * d * d));
      }
    }
  }
}
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
    ...
    if (NON_BAROTROPIC_EOS) {
      ...
      msg << "### NON_BAROTROPIC_EOS" << std::endl;
      
    } else {
      iso_cs = pin->GetReal("hydro", "iso_sound_speed");
    }
    // Get the Hall parameter
    k_wave = pin->GetOrAddReal("problem", "k_wave", 0.0);
    eta_hall = pin->GetOrAddReal("problem", "eta_hall", 0.0);
    
    bx0 = 0.1/std::sqrt(4.0*M_PI);
    by0 = 0.001/std::sqrt(4.0*M_PI);
    bz0 = 0;
    d0  = 1.0;
    p0  = 0.00001;//1.0;
    CA0 = std::sqrt((0.1*0.1)/(4.0*M_PI*d0));
    sigma = eta_hall/2.0*k_wave*k_wave;
    omega = std::sqrt(CA0*CA0*k_wave*k_wave + sigma*sigma);
    dv0 = k_wave/d0*bx0*by0*(-omega/(omega*omega-sigma*sigma));
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    // read Hall effect coefficient
    eta_Hall = pin->GetOrAddReal("problem", "eta_hall", 0.0);
    if (eta_Hall != 0.0)
        EnrollFieldDiffusivity(Hall_coefficient);
  }
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    */
    ...
    for (int k=ks; k<=ke; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie+1; i++) {
            pfield->b.x1f(k,j,i) = bx0;//(a3(k  ,j+1,i) - a3(k,j,i))/pcoord->dx2f(j) -
    ...
    for (int k=ks; k<=ke; k++) {
      for (int j=js; j<=je+1; j++) {
        for (int i=is; i<=ie; i++) {
            pfield->b.x2f(k,j,i) = by0*std::cos(2.0*M_PI*k_wave*pcoord->x1v(i));//(a1(k+1,j,i  ) - a1(k,j,i))/pcoord->dx3f(k) -
    ...
    for (int k=ks; k<=ke+1; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie; i++) {
            pfield->b.x3f(k,j,i) = bz0;//(a2(k,j  ,i+1) - a2(k,j,i))/pcoord->dx1f(i) -
    ...
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        ...
        phydro->u(IDN,k,j,i) = d0;
          phydro->u(IM1,k,j,i) = 0;//d0*dv0*std::cos(2.0*M_PI*k_wave*pcoord->x3v(k));
          phydro->u(IM2,k,j,i) = 0;//d0*dv0*std::cos(2.0*M_PI*k_wave*pcoord->x1v(i));
          phydro->u(IM3,k,j,i) = 0;//d0*dv0*std::sin(2.0*M_PI*k_wave*pcoord->x1v(i))*sigma/omega;

        if (NON_BAROTROPIC_EOS) {
            phydro->u(IEN,k,j,i) = p0/gm1 ;//+ 0.5*d0*dv0*dv0*(std::cos(2.0*M_PI*k_wave*pcoord->x1v(i))*std::cos(2.0*M_PI*k_wave*pcoord->x1v(i)) + (std::sin(2.0*M_PI*k_wave*pcoord->x1v(i))*sigma/omega*std::sin(2.0*M_PI*k_wave*pcoord->x1v(i))*sigma/omega)) ;
          if (MAGNETIC_FIELDS_ENABLED) {
            phydro->u(IEN,k,j,i) += 0.5*(bx0*bx0+by0*std::cos(2.0*M_PI*k_wave*pcoord->x1v(i))*by0*std::cos(2.0*M_PI*k_wave*pcoord->x1v(i))+bz0*bz0);
          }
        }
      }
    }
